...
## State as of 2026-09-13 — SUPERSEDED, see `docs/STATUS.md`

**Moved here verbatim from `CLAUDE.md`'s "Current state" section on
2026-09-15, unedited.** That section was 351 lines — a third of the file — and
it was hand-written: 58 date stamps and 82 point estimates describing which
tables were current, what each noise floor was, and what was staged. Keeping it
accurate was the largest single per-session cost in the project, and three of
the six noise floors it quoted had already drifted a digit from the tables they
described, while its staged-swap list still showed five where the ledger had
four (the Rendmaw swap moved to `WITHDRAWN` on 2026-09-13 and this section was
never updated).

It is replaced by `docs/STATUS.md`, which is GENERATED from the artefacts
themselves — table headers, cache fingerprints, the `edhmc/pending` ledger, the
`KNOWN_ISSUES.md` index, the modules on disk — by `python -m tools.status
--write`. Nothing in that file is remembered, so nothing in it can go stale
without the thing it describes having moved.

**Everything below was true when it was written.** It is kept because the
reasoning is the durable part: which regeneration moved which rows, why
Shilgengar coming back byte-identical was the load-bearing check, and what each
mover was attributable to. For a number, read the tables in `results/`.
...
## Queued items closed before 2026-09-15 — moved from `CLAUDE.md`

**Moved here verbatim on 2026-09-15**, following the precedent the queued-work
section set for itself ("Items closed before 2026-09-07 have been moved to
`docs/HISTORY.md` with their evidence"). Every entry below is DONE, WITHDRAWN
or superseded; each names the `§` in `KNOWN_ISSUES.md` that carries its
evidence, and those ids are stable. `CLAUDE.md`'s queued-work section now
carries the OPEN items only.

The `-old` entries are kept for the same reason they were kept before: the
original reasoning is evidence about how a wrong conclusion was reached, and
this project has twice recovered a real finding from one.
...
with "read this before anything else" and was three days stale relative to
`CLAUDE.md` when it was moved: it described the 2026-09-12 fourteen-fix batch
as the current frontier, listed the Rendmaw swap as awaiting a decision when
the ledger had already moved it to `WITHDRAWN`, and told the reader all six
tables still needed regenerating after they had been.

`HANDOFF.md` is orientation for a human arriving cold; it now points at
`docs/STATUS.md` for state rather than carrying a copy of it. The reasoning
below — what each of the fourteen defects was, and why the resulting win-rate
drops are accuracy rather than decks getting worse — is the durable part.
